        ...
        withdraw(req.user.id, amount, destination, withdrawalId, function(err) {
            if (err) {
                if (err === 'NOT_ENOUGH_MONEY')
                    ...
                    return res.render('withdraw_request', { user: user,  id: uuid.v4(), warning: 'Please make sure you entered a valid address without any spaces! Another reason might be, that the hot wallet is offline or empty. Please try again later or contact the support.' });
